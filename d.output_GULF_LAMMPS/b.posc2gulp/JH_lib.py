...
4. write structure
    ...
    4.3 w_gulp_str(filename, unitcell, compound, position, Selective):
    : To write LAMMPS crystal structure file type, 
...
def w_gulp_str(compound, position, filename = None, unitcell = None, sequence_of_compound = None, potential = None):
    """
    4.2 w_lammps_str(filename, unitcell, compound, position, Selective):
    : To write LAMMPS crystal structure file type, 
    Officially, it needs 2 parameters. "compound and position"
    ...
    f=open(filename, 'w')
    f.write('opti pro conp\n   cell\n    %13.8f%13.8f%13.8f%5.1f%5.1f%5.1f\nfractional\n' 
            %( float(unitcell[0][0]), float(unitcell[1][1]), float(unitcell[2][2]), 90, 90, 90 )
            )
    i = 1
    ...
    for a in sequence_of_compound[0]:
        for b in position:
            if a == b[0]:
                f.write('%5.2s core   %18.17f  %18.17f  %18.17f\n' 
                    %( b[0], float(b[1])/float(unitcell[0][0]), float(b[2])/float(unitcell[1][1]), float(b[2])/float(unitcell[2][2]))
                    )
    f.write('species\n')
    for a in range(len(sequence_of_compound[0])):
        f.write('%2.2s%8.2s.00\n' %(sequence_of_compound[0][a], sequence_of_compound[1][a])
            )
...
potential = [['Zr','O',1502.11,0.345,5.1,0.0,20.0],
             ['O','O',9547.96,0.224,32.0,0.0,20.0],
             ['Y','O',1366.35,0.348,19.6,0.0,20.0]]
unitcell, compound, position = r_cryst_vasp(target)
w_gulp_str(compound, position, "structure.data", unitcell, sequence_of_compound,potential)
