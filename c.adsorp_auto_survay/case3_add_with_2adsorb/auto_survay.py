...
def bond_length_finding(input_position):
	...
	for a in range(len(input_position)):
		for b in range(a):
			if input_position[a][1:4] != input_position[b][1:4] and input_position[a][0] !=  input_position[b][0]: 
		...
		for a in range(len(input_position)):
			for b in range(a):
				if input_position[a][1:4] != input_position[b][1:4]:
					distance.append(JH_lib.distance_atoms(input_position[a][1:4], input_position[b][1:4]))
		# print "[From the function: bond_length_finding]\n\tThe input positions include only one-kind of atom"
		return  min(distance)
	else:	
		# print "[From the function: bond_length_finding]\n\tThe input positions include only several-kinds of atom"
		return min(distance)
...
(2) Reading the structure informations,
	(2-1) Seperate Slab part (No: %4.3i) and Adsorption (No: %4.3i) 
	(2-2) Check the highest atom in slab (%5.4f) in unit of Angstrom
	(2-3) Calculate the bond-length of slabs (%5.4f) in unit of Angstrom'''\
	%(len(slab), len(adsorp), temp_high,bonds_length)
# -----------------------------------------------------------------------------
...
for a in slab:
	temp = 0
	for b in expd_slab:
		if JH_lib.distance_atoms(a[1:4], b[1:4]) <  bonds_length * 1.2 and a[0] != b[0]:
			temp = temp + 1
	N_bb.append(temp)
